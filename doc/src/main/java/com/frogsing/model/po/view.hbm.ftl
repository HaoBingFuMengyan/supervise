...
<hibernate-mapping xmlns="urn:nhibernate-mapping-2.2">
	...
	<#if (field.code?upper_case=="IVERSIONNO" )>
		${"<"}version column="${field.code?lower_case}"  name="${field.code?lower_case}" type="<@fieldToHiberateType field=field/>" ${"/>"}
		<#break/>
	</#if>
	</#list>
	<#list entity.columns as field>
	<#if (field.code?upper_case!="SGUID" && field.code?upper_case!="IVERSIONNO")>
		${"<"}property column="${field.code?lower_case}" name="${field.code?lower_case}" type="<@fieldToHiberateType field=field/>" ${"/>"}
	</#if>
	</#list>
	 ${frogsingcode[0]!""}
	</class>
