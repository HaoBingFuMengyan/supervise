    ...
    var urls = [
        <c:choose>
            <c:when test="${param.type == 0}">
                ["收票明细", "${ctx}/ht/receiptInfo/receivelist.shtml?scontractid=${id}"],
                ["开票明细", "${ctx}/ht/receiptInfo/invoicelist.shtml?scontractid=${id}"]
            </c:when>
            <c:otherwise>
                ["收款明细", "${ctx}/fund/billpay/receivabled.shtml?scontractid=${id}"],
                ["付款明细", "${ctx}/fund/billpay/payment.shtml?scontractid=${id}"]
            </c:otherwise>
        </c:choose>
