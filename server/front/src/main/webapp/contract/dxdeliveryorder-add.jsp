            ...
            layui.use(['layer', 'form','element','table','laydate'], function(){
                ...
                table.render({
                    ...
                    ,cols: [[
                        ...
                        // ,{field:'favailableweight',width:'10%',align:'right',title: '剩余', sort: true, templet:function(d){
                        //         return d.flockweight;
                        //     }}
                        ,{width:'10%',align:'right',title: '已开单量', templet:function(d){
                                var foderweight=0;
                                if(data.deliverydetailmap!=undefined){
                                    foderweight = data.deliverydetailmap[d.id].foderweight;
                                }
                               return data.map[d.id].forderweight-foderweight;
                        }}
                        ,{field:'fcanorderweight',align:'right',title: '可开单量', sort: true,templet:function(d){
                                ...
                                return data.map[d.id].fcanorderweight+foderweight;
                            }}
                        ,{type:'space',width:'15%',title:'开单量',align:'right',edit:'button',style:'background-color:antiquewhite;', unresize: true,templet:function(d){
                            var foderweight=0;
                            if(data.deliverydetailmap!=undefined){
                                foderweight = data.deliverydetailmap[d.id].foderweight;
                            }
                ...
                table.on('edit(detail)', function(obj){
                    ...
                    var value = obj.value //得到修改后的值
                        ...
                        $("#formx").prepend("<input type='hidden' name='"+
                                n(d.id,"foderweight")+
                                "' data-max2='"+d.flockweight+
                            "' data-max='"+data.map[d.id].fcanorderweight+"' data-type='inputweight' value='"+obj.value+"'>");
                        $("#formx").prepend("<input type='hidden' name='"+
                            n(d.id,"scontractdetailid")+ "' value='"+data.map[d.id].id+"'>");
...
<form action="${ctx}/contract/deliveryorderadd.json" method="post" id="formx" class="layui-form" lay-size="sm" lay-filter="formDemo">
    ...
    <div class="layui-form-item">
        ...
        <div class="layui-inline">
            ...
            <div class="layui-input-inline">
                <input type="text" id="dvaliddate" name="dvaliddate" value="<mv:format label="date" value="${deliveryOrder.dvaliddate}"/>" placeholder="必填项" class="layui-input required" autocomplete="off" readonly/>
            </div>
        </div>
        <div class="layui-inline">
            <label class="layui-form-label">联系手机： </label>
            <div class="layui-input-inline">
                <input type="text" id="smobile" name="smobile" value="${deliveryOrder.smobile}" placeholder="必填项" class="layui-input" autocomplete="off"/>
            </div>
        </div>
