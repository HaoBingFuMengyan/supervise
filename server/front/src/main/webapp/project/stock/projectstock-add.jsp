        ...
        $().ready(function () {
            ...
            $("#addDetail").click(function(){
                ...
                layer.open({
                    ...
                    title: '添加项目明细',
                    area: ['90%', '400px'],
                    content: ['${ctx}/projectstock/detailadd.shtml', 'no'] //这里content是一个URL，如果你不想让iframe出现滚动条，你还可以content: ['http://sentsin.com', 'no']
                });
            });
...
//            cellRate();
        ...
        function repaymentList(){
            $.ajax({
                type: "post",
                url: "${ctx }/repaymentplan/payments.json",
                data : {"id":'${obj.id}'},
                success: function (data) {
                    var repaymentPlans = data.obj;
                    innerRepaymentPlanHtml(repaymentPlans);
                },
                error: function (data) {
                }
            });
...
//            var floanamount = $("#floanamount").val();//贷款总额
            var floanamount = $("#fsaleamount").val();//修改之后：生成还款计划周期的项目总额取销售总额
            ...
            if (repalce){
                if(isNull(floanamount)||
                    ...
                    var repaymentPlanHtml="<tr id='repaymentPlanHtml1'>" +
                        "<td>1<input type='hidden' name='payments[0].ino' value='1'/></td>" +
                        "<td><input type='text' data-name='dplandate' name='payments[1].dplandate' placeholder='(格式：2019-01-01)' class='layui-input required dateISO' autocomplete='off'/></td>" +
                        "<td><input type='text' data-name='fplanback' data-index='1' name='payments[0].fplanback' placeholder='(必填项)' class='layui-input required number' autocomplete='off' onchange='CalculAmount(this)'/></td>" +
                        "<td><input type='text' data-index='1' data-name='freceivabletotal' name='payments[1].freceivabletotal' class='layui-input readonly' readonly='readonly' autocomplete='off'/></td>" +
                        //                        "<td><input type='text' name='' class='layui-input readonly' readonly='readonly' autocomplete='off'/></td> " +
                        "<td><input type='text' data-index='1' data-name='fless' name='payments[0].fless' class='layui-input readonly' readonly='readonly' autocomplete='off'/></td> " +
                        "<td><input type='text' data-name='sremark' name='payments[0].sremark' class='layui-input' autocomplete='off'/></td> " +
                ...
                if (fterminallyrefundrate != 0 && !isNull(fterminallyrefundrate) && !isNull(dintereststart)){
                    $.ajax({
                        type: "post",
                        url: "${ctx }/projectstock/calculateEarnings.json",
                    ...
                    $.ajax({
                        type: "post",
                        url: "${ctx }/projectstock/repaymentlist.json",
                        data : {"id":'${obj.id}'},
                        success: function (data) {
                            var repaymentPlans = data.obj;
                            innerRepaymentPlanHtml(repaymentPlans);
...
<div class="mbody">
    <mw:msg/>
    <form class="layui-form" action="${ctx }/projectstock/savestock.shtml" method="post" id="formx" onkeydown="if(event.keyCode==13) return false;"
        ...
        <div class="layui-tab layui-tab-card">
            ...
            <div class="layui-tab-content" style="height: 100%;">
                <div id="projectInfo" class="layui-tab-item layui-show">
                        ...
                        <div class="layui-inline">
                            ...
                            <div class="layui-input-inline">
                                <input id="swarehousecontractno" type="text" name="swarehousecontractno"
                                       value="${obj.swarehousecontractno}"
                                       lay-verify="required"  class="layui-input required" autocomplete="off"/>
                            </div>
                        </div>
                    </div>
                    ...
                    <%--<div class="layui-input-inline">--%>
                    <%--<input type="text" id="fcontractrate" name="fcontractrate" value='<mw:format label="percent" value="${obj.fcontractrate }"/>' class="layui-input money" autocomplete="off"/>--%>
                    <%--</div>--%>
                    <%--<div class="layui-form-mid layui-word-aux">%</div>--%>
                    <%--</div>--%>
                    <%--<div class="layui-inline">--%>
                    ...
                    <%--<div class="layui-input-inline">--%>
                    <%--<input onchange="repalceFun()"  type="text" id="icycle" name="icycle" value='<mw:format label="toint" value="${obj.icycle }"/>' placeholder="(必填项)"  class="layui-input required gt0 onumber" autocomplete="off"/>--%>
                    <%--</div>--%>
                    <%--<div class="layui-form-mid layui-word-aux">月</div>--%>
                    <%--</div>--%>
                    <%--&lt;%&ndash;<div class="layui-inline">&ndash;%&gt;--%>
                    <%--&lt;%&ndash;<label class="layui-form-label">计息日 </label>&ndash;%&gt;--%>
                    <%--&lt;%&ndash;<div class="layui-input-inline">&ndash;%&gt;--%>
                    <%--&lt;%&ndash;<input type="text" id="dintereststart" name="dintereststart"&ndash;%&gt;--%>
                    <%--&lt;%&ndash;value='<mw:format label="date" value="${obj.dintereststart }" format="yyyy-MM-dd"></mw:format>' required lay-verify="required" placeholder="(必填项)"   class="layui-input" autocomplete="off"/>&ndash;%&gt;--%>
                    <%--&lt;%&ndash;</div>&ndash;%&gt;--%>
                    <%--&lt;%&ndash;</div>&ndash;%&gt;--%>
                    <%--</div>--%>
                    ...
                    <div class="layui-form-item">
                        <div class="layui-inline">
                            ...
                            <div class="layui-input-inline">
                                <input type="text" id="ffineinterestrate" name="ffineinterestrate" value='<mw:format label="percent" value="${obj.ffineinterestrate }"/>' placeholder="(必填项)" class="layui-input required gt0 money" autocomplete="off"/>
                            </div>
                            <div class="layui-form-mid layui-word-aux">%</div>
                        </div>
                        <div class="layui-inline">
                            ...
                            <div class="layui-input-inline">
                                <input onchange="repalceFun()" type="text" id="irefundcycle" name="irefundcycle" value='<mw:format label="toint" value="${obj.irefundcycle }"/>' placeholder="(必填项)"  class="layui-input required gt0 onumber" autocomplete="off"/>
                            </div>
                            <div class="layui-form-mid layui-word-aux">月</div>
                        </div>
                    </div>
                    ...
                    <%--<div class="layui-input-inline">--%>
                    <%--<input onchange="repalceFun()" type="text" id="fterminallyrefundrate" name="fterminallyrefundrate" value='<mw:format label="percent" value="${obj.fterminallyrefundrate }"/>' class="layui-input money" autocomplete="off"/>--%>
                    <%--</div>--%>
                    <%--<div class="layui-form-mid layui-word-aux">%</div>--%>
                    <%--</div>--%>
                    <%--<div class="layui-inline">--%>
                    ...
                    <%--<div class="layui-input-inline">--%>
                    <%--<input type="text" id="ipostponemonth" name="ipostponemonth" value='<mw:format label="toint" value="${obj.ipostponemonth }"/>' class="layui-input onumber" autocomplete="off"/>--%>
                    <%--</div>--%>
                    <%--<div class="layui-form-mid layui-word-aux">月</div>--%>
                    <%--</div>--%>
                    <%--</div>--%>
