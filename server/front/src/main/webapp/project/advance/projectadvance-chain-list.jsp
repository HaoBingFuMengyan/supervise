...
<script>
    ...
    layui.use(['table','layer'],function(){
        ...
        jj.url="${ctx}/projectadvance/advance-list.json";
...
//            }}
            ...
            ,{field:'fadvanceamount', title: '预付款金额(元)',width:120,sort: true,align:'right'}
            ,{field:'fbondamount', title: '质保金金额(元)',width:120,sort: true,align:'right'}
        ...
        jj.add=function () {
            jj.open({
                area: ['90%', '90%'],
                title:'项目编辑',
                content:"${ctx}/projectadvance/add.shtml?type=10&isedit=false"
            });
        ...
        jj.chain_over=function(d) {
            top.layer.confirm('确定对该项目进行完结吗?', {icon: 3, title:'系统提示'},function(index){
                jj.post("${ctx}/projectadvance/chain_over.json",{id:d.id},function (rs) {
                    if(rs.success){
                        if(rs.msg){
                            jj.msg(rs.msg);
                        }
        ...
        jj.edit=function (d) {
            jj.open({
                area: ['90%', '90%'],
                title:'项目编辑',
                content:"${ctx}/projectadvance/edit_"+d.id+".shtml?isedit=true"
            });
        ...
        jj.commit=function(d) {
            top.layer.confirm('确定要提交审核吗?', {icon: 3, title:'系统提示'},function(index){
                jj.post("${ctx}/projectadvance/commit.json",{id:d.id},function (rs) {
                    if(rs.success){
                        if(rs.msg){
                            jj.msg(rs.msg);
                        }
...
<script type="text/html" id="xtoolbar">
    <jj:btn event="add" name="发起项目" purview="projectadvance:add"/>
</script>
