            ...
            <div class="layui-tab-content" style="height: 100%;">
                ...
                <div  class="layui-tab-item layui-show">
                    ...
                    <fieldset class="layui-elem-field layui-field-title" style="margin-top: 20px;">
                        <legend>收款计划表<c:if test="${obj.istatus eq 100}"><a @click="addPlan('receipts')" class="layui-btn layui-btn-sm" style="margin-left: 20px;">增加收款计划</a></c:if></legend>
                    </fieldset>
                    <table class="layui-table" >
                        ...
                        <tr v-for="(receipt,index) in receipts">
                            ...
                            <td>{{receipt.brefund==0?'否':'是'}}</td>
                            <td><input type='text' :name="'receipts['+index+'].sremark'" v-model="receipt.sremark" class='layui-input' autocomplete='off'/></td>
                            <td>
                                <a v-if="${obj.istatus eq 100}" v-on:click="del(index,'receipts')" class='layui-abtn demo-delete'>删除</a>
                                <a v-if="receipt.isoverdue" v-on:click="buildBill(receipt.id,index)" class='layui-abtn demo-delete'>生成逾期单</a>
                            </td>
                        </tr>
                        </tbody>
                    </table>
            ...
            loadPlants: function () {
                ...
                jj.post("${ctx }/repaymentplan/receipts.json",{"id":'${obj.id}'},function (data) {
                    thice.receipts = data.obj.map(x => {
                        ...
                        x['isoverdue'] = new Date().setHours(0,0,0,0).valueOf()>new Date(x.dplandate.replace(/-/g,"/")).valueOf() && x.istatus!=30;
                    return x;
                });
                });
            },
