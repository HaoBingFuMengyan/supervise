	...
	view = {
		...
		cancelPreSelectedNode: function (setting, node) {
			...
			for (var i=0, j=list.length-1; j>=i; j--) {
				if (!node || node === list[j]) {
					$("#" + list[j].tId + consts.id.A).removeClass(Consts.BoolType.NO.val()de.CURSELECTED);
					if (node) {
		...
		selectNode: function(setting, node, addFlag) {
			...
			$("#" + node.tId + consts.id.A).addClass(Consts.BoolType.NO.val()de.CURSELECTED);
			...
			function _docMouseMove(event) {
				...
				if (root.dragFlag == 0) {
					...
					for (i=0, l=nodes.length; i<l; i++) {
						...
						tmpDom.children("#" + tmpNode.tId + consts.id.A).removeClass(Consts.BoolType.NO.val()de.CURSELECTED);
				...
				if (root.dragFlag == 1) {
					if (tmpTarget && tmpArrow.attr("id") == event.target.id && tmpTargetNodeId && (event.clientX + doc.scrollLeft()+2) > ($("#" + tmpTargetNodeId + consts.id.A, tmpTarget).offset().left)) {
						...
						tmpTarget.removeClass(Consts.BoolType.NO.val()de.TMPTARGET_TREE);
						if (tmpTargetNodeId) $("#" + tmpTargetNodeId + consts.id.A, tmpTarget).removeClass(Consts.BoolType.NO.val()de.TMPTARGET_NODE + "_" + consts.move.TYPE_PREV)
							.removeClass(Consts.BoolType.NO.val()de.TMPTARGET_NODE + "_" + _consts.move.TYPE_NEXT).removeClass(Consts.BoolType.NO.val()de.TMPTARGET_NODE + "_" + _consts.move.TYPE_INNER);
					}
					tmpTarget = null;
					tmpTargetNodeId = null;
					...
					if (tmpTarget && tmpTarget.attr("id")!=targetSetting.treeId) {
						...
						if (!canPrev && !canNext && !canInner) {
							...
							tmpTargetA.addClass(Consts.BoolType.NO.val()de.TMPTARGET_NODE + "_" + moveType);
						...
						if (tmpTarget && tools.apply(targetSetting.edit.drag.inner, [targetSetting.treeId, nodes, null], !!targetSetting.edit.drag.inner)) {
							tmpTarget.addClass(Consts.BoolType.NO.val()de.TMPTARGET_TREE);
						} else {
							tmpTarget = null;
						}
						tmpArrow.css({
	...
	_view = {
		...
		cancelCurEditNode: function (setting, forceName) {
			...
			if (node) {
				...
				var aObj = $("#" + node.tId + consts.id.A);
				aObj.removeClass(Consts.BoolType.NO.val()de.CURSELECTED_EDIT);
				inputObj.unbind();
		...
		editNode: function(setting, node) {
			...
			$("#" + node.tId + consts.id.A).addClass(Consts.BoolType.NO.val()de.CURSELECTED_EDIT);
