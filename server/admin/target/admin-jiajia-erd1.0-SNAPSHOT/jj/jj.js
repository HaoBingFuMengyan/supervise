(function (jj) {
    ...
    jj.tab=function(config){
        if(top.triggerTab) {
            top.triggerTab(config);
        }else {
    ...
    jj.render=function (config) {
        ...
        /*if(!x.height){
            x=$.extend(x,{ height:'full-'+($('.pull-left').offset().top+$('.pull-left').height()+20)});
        }*/
        x.url=jj.url;
        x.cols=jj.cols;
        var w=jj.where();
...
//            console.log(this); //当前排序的 th 对象
    ...
    jj.post=function(url,data,callback){
        $.ajax({
            ...
            error:function (textStatus) {//请求失败后调用的函数
                this.success(textStatus.responseJSON);
            },
            dataType: "json"
        });
        jj.loading();
    ...
    jj.getUrl=function(u){
        var url=jj.url;
        if(jj.xurl)
            url=jj.xurl;
        var a=url.lastIndexOf("/");
        return url.substring(0,a+1)+u;
    };
    jj.add=function(){
        var title=$(this).prop('title');
        var config={
            title:title?title:'',
            content: jj.getUrl("detail.shtml")
        };
        var i=jj.xopen(config);
    };
    jj.detail=function(obj){
        var title=$(this).prop('title');
        var i=jj.xview({
            title:title?title:'',
            content: jj.getUrl("detail.shtml")+"?id="+obj.id
        });
    ...
    jj.delete=function (obj) {
        var title=$(this).prop('title');
        var i = layer.confirm( title?title:'确定要删除吗?', {icon: 3, title:'系统提示'},function(index){
            layer.close(i);
            jj.post(jj.getUrl("delete.json"),{ids:[obj.id]});
        });

    };
    jj.deleteAll=function () {
        if (jj.ids().length > 0)
            var i = layer.confirm("确定要删除选中行吗?", {icon: 3, title: '系统提示'}, function (index) {
                layer.close(i);
                jj.post(jj.getUrl( "delete.json"), {ids: jj.ids()});
            });
    };
...
(function (tt) {
    ...
    tt.readonly=function(){
        $(document).ready(function () {
            ...
            }).prop('readonly',true).css('height','30px');
    ...
    tt.Format=function(datetime,fmt) {
    ...
    tt.readonly=function(){
        $(document).ready(function () {
            $('input[type="text"]').css('width','auto').css('border',0).css('background-color','#ffffff').each(function(){
                $(this).val($(this).val()+$(this).parent().next().text());
                $(this).parent().next().text('');
