...
public class StringHelper {
	 ...
	 * @return
	 */
	public static final String replaceAllStr(String source,String replaceStr){
		Pattern p = Pattern.compile("<span.*style=\"text-decoration: line-through;\".*</span>");
		Matcher m = p.matcher(source);
		//去除标签
		String result = m.replaceAll(replaceStr);
		return result;
	}
	 ...
	 * @return
	 */
	public static String fillString(String fill,String str, int length) {
		if (str == null) {
			str = "";
		}
		if(str.length() > length)
			return str.substring(0,length);
		int strLen = charLength(str);
		for (int i = 0; i < length - strLen; i++) {
			str += fill;
		}
		return str;
	}
	public static String fillStringEnd(String fill,String str, int length) {
		if (str == null) {
			str = "";
		}
		if(str.length() > length)
	 ...
	 * @return
	 */
	public static String linkedArray(String[] arr,String split){
		if(arr == null || arr.length == 0)
			return "";
		String tmp="";
		for(int i=0;i<arr.length;i++){
			tmp=tmp.concat(split).concat(arr[i]);
		}
		return tmp.substring(split.length());
	}
}