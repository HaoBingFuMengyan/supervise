...
public class HttpProtocolHandler {
    ...
    public HttpResponse execute(HttpRequest request, String strParaFileName, String strFilePath) throws HttpException, IOException {
        ...
        } else if(B.Y(strParaFileName) && B.Y(strFilePath)) {
        	//post模式且不带上传文件
        ...
        try {
            ...
            if("application/octet-stream".equals(method.getResponseHeader("Content-Type"))){
            	response.setByteResult(method.getResponseBody());
            }else{
            	if (request.getResultType().equals(HttpResultType.STRING)) {
            		response.setStringResult(URLDecoder.decode(method.getResponseBodyAsString(), charset));
