...
public class ThreadUtils {
	...
	static {
		if (executor == null) {
			try {
				executor = new ThreadPoolExecutor(Integer.parseInt(Props
						.get("Thread.PoolSize")), Integer.parseInt(Props
						.get("Thread.MaxPoolSize")), Integer.parseInt(Props
						.get("Thread.KillTime")), TimeUnit.MINUTES,
						new LinkedBlockingQueue<Runnable>());
				/*
				 * executor = new ThreadPoolExecutor(3, 5, 1, TimeUnit.MINUTES,
				 * new LinkedBlockingQueue<Runnable>());
				 */
			} catch (Exception e) {
				e.printStackTrace();
